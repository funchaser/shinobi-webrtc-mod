...
//webrtc*mod              
				if (e.details.detector !== '0')
                {
                  ffmpegCommand.push(commandStringPart)
                }
              	else
                {
                  if (commandStringPart.search("loglevel") > 0)
                  {ffmpegCommand.push(" -loglevel warning -r 1 -stream_loop -1 -i "+__dirname+"/../web/libs/img/poster.mp4 -vf fps=1,realtime ")}
                  else
                  {ffmpegCommand.push(commandStringPart)}
                } 
//webrtc*mod              
